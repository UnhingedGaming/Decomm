# T480S rescue - two modes:
#   -Mode Gather : finds every real file on C: that isn't Windows or a program (wherever
#                  OneDrive buried it) and MOVES it into Desktop\T480S-Gathered, keeping its
...
#                  Every move is recorded in T480S-Gathered\_undo-list.csv.
#   -Mode Usb    : copies Desktop\T480S-Gathered onto one or more USB sticks, filling them in order.
...
$gathered = Join-Path $env:USERPROFILE 'Desktop\T480S-Gathered'
...
$skipNames = @('AppData', 'Application Data', 'Local Settings', 'IntelGraphicsProfiles', 'T480S-Rescue',
    'All Users', 'Default', 'Default User', '$WINDOWS.~BT', '$Windows.~WS', '$SysReset')
if ($Mode -eq 'Gather') { $skipNames += 'T480S-Gathered' }
...
$first = $targets[0].Letter + ':\T480S-Rescue'
...
function Test-Copied($rel, $len) {
    foreach ($t in $targets) {
        $e = New-Object System.IO.FileInfo ('\\?\' + $t.Letter + ':\T480S-Rescue\' + $rel)
...
foreach ($f in $files) {
    ...
    for ($k = $ti; $k -lt $targets.Count -and -not $placed; $k++) {
        ...
        $to = '\\?\' + $t.Letter + ':\T480S-Rescue\' + $f.Rel
        try {
            [System.IO.Directory]::CreateDirectory([System.IO.Path]::GetDirectoryName($to)) | Out-Null
