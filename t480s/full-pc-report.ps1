# T480S FULL PC REPORT - READ-ONLY. Reads only file names and sizes on the whole C: drive.
# Opens no files, changes nothing, downloads nothing.
# Output on the Desktop (C:\Users\<you>\Desktop):
#   T480S-Full-Report.txt   - summary: where every GB is, where personal files are hiding
#   T480S-All-Folders.csv   - every folder on C: with real size, cloud-only size, file count
param([string]$Drive = 'C:\')

$desk = Join-Path $env:USERPROFILE 'Desktop'
$report = Join-Path $desk 'T480S-Full-Report.txt'
$csv = Join-Path $desk 'T480S-All-Folders.csv'
...
Add "T480S FULL PC REPORT - $(Get-Date) - read-only"
