# X51 FULL PC REPORT - READ-ONLY. Reads only file names and sizes on the whole C: drive.
# Opens no files, changes nothing, downloads nothing.
# Output on the Desktop (C:\Users\<you>\Desktop):
#   X51-Full-Report.txt   - summary: where every GB is, where personal files are hiding
#   X51-All-Folders.csv   - every folder on C: with real size, cloud-only size, file count
param([string]$Drive = 'C:\')

$desk = Join-Path $env:USERPROFILE 'Desktop'
$report = Join-Path $desk 'X51-Full-Report.txt'
$csv = Join-Path $desk 'X51-All-Folders.csv'
...
Add "X51 FULL PC REPORT - $(Get-Date) - read-only"
