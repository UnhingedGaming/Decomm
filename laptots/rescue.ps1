# LAPTOTS rescue - two modes:
#   -Mode Gather : finds every real file on C: that isn't Windows or a program (wherever
#                  OneDrive buried it) and MOVES it into Desktop\LAPTOTS-Gathered, keeping its
...
#                  Every move is recorded in LAPTOTS-Gathered\_undo-list.csv.
#   -Mode Usb    : copies Desktop\LAPTOTS-Gathered onto one or more USB sticks, filling them in order.
...
$gathered = Join-Path $env:USERPROFILE 'Desktop\LAPTOTS-Gathered'
...
$skipNames = @('AppData', 'Application Data', 'Local Settings', 'IntelGraphicsProfiles', 'LAPTOTS-Rescue',
    'All Users', 'Default', 'Default User', '$WINDOWS.~BT', '$Windows.~WS', '$SysReset')
if ($Mode -eq 'Gather') { $skipNames += 'LAPTOTS-Gathered' }
...
$first = $targets[0].Letter + ':\LAPTOTS-Rescue'
...
function Test-Copied($rel, $len) {
    foreach ($t in $targets) {
        $e = New-Object System.IO.FileInfo ('\\?\' + $t.Letter + ':\LAPTOTS-Rescue\' + $rel)
...
foreach ($f in $files) {
    ...
    for ($k = $ti; $k -lt $targets.Count -and -not $placed; $k++) {
        ...
        $to = '\\?\' + $t.Letter + ':\LAPTOTS-Rescue\' + $f.Rel
        try {
            [System.IO.Directory]::CreateDirectory([System.IO.Path]::GetDirectoryName($to)) | Out-Null
